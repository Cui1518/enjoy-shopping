...
import com.changgou.user.feign.UserFegin;
...
//    @GlobalTransactional(name = "order_add")
    ...
    public String  add(Order order){
        ...
        order.setSourceType("1");  //1:访问来源web
        order.setOrderStatus("0");  //0:未完成  1:已完成  2:已退货
        order.setPayStatus("0");  //0:未支付  1:已支付
        order.setConsignStatus("0");  //0:为发货  1:已发货
        String orderId=idWorker.nextId()+"";
        order.setId(orderId);
        orderMapper.insertSelective(order);
        //4.填充订单项数据并保存到tb_order_item
        for (OrderItem orderItem : orderItemList) {
            orderItem.setId(idWorker.nextId()+"");//通过雪花算法设置id
    ...
    @Override
    @Transactional
    public void mergeOrder(Long zid, Long cid) {
        //获取从订单数据
        Order corder = orderMapper.selectByPrimaryKey(cid);
        Integer ctotalNum = corder.getTotalNum();
        Integer ctotalMoney = corder.getTotalMoney();
        //获取主订单数据
        Order zorder = orderMapper.selectByPrimaryKey(zid);
        //获取主订单item
        OrderItem orderItem = new OrderItem();
        orderItem.setOrderId(zorder.getId());
        List<OrderItem> zorderItemList =  orderItemMapper.select(orderItem);
        for (OrderItem zorderItem : zorderItemList) {
            String zskuId = zorderItem.getSkuId();
            //获取从订单item
            OrderItem orderItem1 = new OrderItem();
            orderItem1.setOrderId(corder.getId());
            List<OrderItem> corderItemList =  orderItemMapper.select(orderItem1);
            for (OrderItem corderItem : corderItemList) {
                String cskuId = corderItem.getSkuId();
                //如果商品相同,进行数量和付款价格的合并
                if (cskuId.equals(zskuId)){
                    zorderItem.setNum(zorderItem.getNum()+corderItem.getNum());
                    zorderItem.setPayMoney(zorderItem.getPayMoney()+corderItem.getPayMoney());
                    orderItemMapper.updateByPrimaryKey(zorderItem);
                    //删除从订单item
//                    orderItemMapper.deleteByPrimaryKey(corderItem.getId());
                }else {
                    //商品不相同,更改为主订单id
                    corderItem.setId(zorderItem.getId());
                    orderItemMapper.updateByPrimaryKeySelective(corderItem);
                }
            }
        }
        //更新主订单数据
        Integer ztotalNum = zorder.getTotalNum();
        Integer ztotalMoney = zorder.getTotalMoney();
        zorder.setTotalNum(ztotalNum+ctotalNum);
        zorder.setTotalMoney(ztotalMoney+ctotalMoney);
        orderMapper.updateByPrimaryKeySelective(zorder);
        //逻辑删除从订单
        corder.setIsDelete("1");
        orderMapper.updateByPrimaryKeySelective(corder);
    ...
    public void closeOrder(String orderId) {
        //1.根据订单id查询mysql中的订单信息,判断订单是否存在,同时判断订单的支付状态
        Order order = orderMapper.selectByPrimaryKey(orderId);
        if (order==null){
    ...
    public void autoTack() {
        ...
        //2.得到当前时间点向前数(订单自动确认的时间点)天,作为过期的时间节点
