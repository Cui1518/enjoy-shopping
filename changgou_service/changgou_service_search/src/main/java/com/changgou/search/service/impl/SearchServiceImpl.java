...
public class SearchServiceImpl implements SearchService {
    ...
    public Map search(Map<String, String> searchMap) {
        ...
        if (searchMap!=null){
            ...
            if (StringUtils.isNotEmpty(searchMap.get("price"))){
                ...
                if (prices.length==2){
                    /**
                     * rangeQuery()区间过滤,price操作的索引库域名
                     * lte 小于等于     lte(prices[1])小于等于价格数组中的第二个元素
                     */
            ...
            /**
             *addAggregation()设置聚合条件
             * terms()设置聚合结果列名
             * field()设置根据哪一个域进行分组查询
             */
            ...
            //按照相关字段进行排序查询
            //前端需传递参数:1.sortField"当前域(根据哪个域进行排序) 2.sortRule当前的 排序操作(升序ASC,降序DESC)
            ...
            HighlightBuilder.Field filed=new HighlightBuilder.Field("name")//当前的高亮域
     ...
     *    }
     */
    public Map<String, Set<String>> formartSpec(List<String> stringList){
        Map<String,Set<String>> resultMap =new HashMap<>();
        if (stringList!=null&&stringList.size()>0){
            for (String specJsonString : stringList) {
                //将json数据转换为map
